...
namespace YuzuDelivery.Umbraco.Core.Startup
{
    public class DefaultUmbracoVmBuilderConfig : YuzuViewmodelsBuilderConfig
    {
            ...
            : base(vmBuilderConfigs)
        {
            ...
            GeneratedViewmodelsOutputFolder = GetModelsDirectory(hostEnvironment.ContentRootPath, vmGenerationSettings.CurrentValue.Directory, vmGenerationSettings.CurrentValue.AcceptUnsafeDirectory);
        }
        ...
        private string GetModelsDirectory(string root, string config, bool acceptUnsafe)
        {
            ...
            if (Path.IsPathFullyQualified(config))
            {

               var dir = Path.GetFullPath(config);
